...
//			$ergeb = mysql_query( $frage ) OR die( "Error: $frage <br>". mysql_error() ) ;
			...
			$abfrage = "UPDATE $db_importbim set
				SSR = REPLACE( SSR, 'ÃŸ', 'ß' ),
				SSR = REPLACE( SSR, 'Ã¤', 'ä' ),
				SSR = REPLACE( SSR, 'Ã¼', 'ü' ),
				SSR = REPLACE( SSR, 'Ã¶', 'ö' ),
				SSR = REPLACE( SSR, 'Ã„', 'Ä' ),
				SSR = REPLACE( SSR, 'Ãœ', 'Ü' ),
				SSR = REPLACE( SSR, 'Ã–', 'Ö' ),
				SSR = REPLACE( SSR, 'â‚¬', '€' )"	;


			$abfrage = "UPDATE $db_importbim set
				Zuordnung = REPLACE( Zuordnung, 'Ã¤', 'ä' ),
				Belegung = REPLACE( Belegung, 'Ã¤', 'ä' ),
				SSR = REPLACE( SSR, 'Ã¤', 'ä' )" ;
//			mysql_query( $abfrage ) OR die( "Error: $abfrage <br>". mysql_error() ) ;
			...
			while( $rw1 = mysql_fetch_object( $erg1 ) ) {
				...
				if( $pos >= 0 ) echo $needle . ' auf ' . $pos . '<br>' ;
				$needle = 'Ã¼' ;
				$pos = strpos( $beleg, $needle ) ;
				if( $pos >= 0 ) echo $needle . ' auf ' . $pos . '<br>' ;
				$needle = 'ch' ;
...
//-----------------------------------------
			...
			while( $rw1 = mysql_fetch_object( $erg1 ) ) {
				...
				$revit	= $rw1->RevitID ;
				$zuord	= $rw1->Zuordnung ;
//				$zuord	= iconv('UTF-8', 'windows-1252', $zuord ) ;
				$top		= $rw1->TOP ;
				$name		= $rw1->Name ;
				$ebene	= $rw1->Ebene ;
				$beleg	= $rw1->Belegung ;
//				$beleg	= iconv('UTF-8', 'windows-1252', $beleg ) ;
				$flch		= $rw1->Flche ;
				$flch		= round( $flch, 2 ) ;
				$ssr		= $rw1->SSR ;
//				$ssr	= iconv('UTF-8', 'windows-1252', $ssr ) ;
