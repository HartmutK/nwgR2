...
if( $ok ) {
	...
	if( isset( $_POST[ 'save' ] ) ) {
		...
		if( $indx <> '' ) {
			if( isset( $_SESSION[ 'merkid' ] ) ) {
				...
				$abfrage = "UPDATE $db_idx SET Indx = '$indx' WHERE IndxID = '$tableid'" ;
				unset( $_SESSION['merkid'] ) ;
				$errortxt = $wastxt . 'ge&auml;ndert' ;
				}
			else {
				$abfrage	= "INSERT INTO $db_idx ( `IndxID`, `Indx` ) VALUES( NULL, '$indx' )" ;
	...
	if(  isset( $_POST[ 'detail' ] ) ) {
		...
		$abfrage = "SELECT * FROM $db_idx WHERE IndxID = '$tableid'" ;
		$ergebnis = mysql_query( $abfrage )  OR die( "Error: $abfrage <br>". mysql_error() ) ;
		$row = mysql_fetch_object( $ergebnis ) ;
		$_SESSION[ 'merkid' ] = $row->IndxID ;
		$widmung = $row->Widmung ;
		$indx = $row->Indx ;
	...
	if( isset( $_POST[ 'del' ] ) ) {
		$tableid = $_POST[ 'del' ] ;
		$abf = "DELETE FROM $db_idx WHERE IndxID = '$tableid'" ;
		...
		<div id="mains">
			<form method="post" Name='input_fields' class="login" enctype="multipart/form-data" >
				<div id="mains_oben">
					<div class="clear"></div>
					<div class="trin">
						<div class="col_80"><input type="text" name="indx" class="col_60" value="<?php echo $indx ; ?>"></div>
				...
				<div id="mains_unten">
					...
					while( $i < $anz_liste ) {
						...
						<div class="clear"></div>
						<div class="tr">
							<div class="col_80"><?php echo $ndx ; ?> </div>
							<div class="col_buttons">
