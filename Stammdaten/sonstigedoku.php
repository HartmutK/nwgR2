	...
	if( isset( $_POST[ 'save' ] ) ) {
		...
		if( $desc <> '' ) {
			if( isset( $_SESSION[ 'merkid' ] ) ) {
				...
				$abfrage	= "INSERT INTO $db_ugsource (`UGID`, `Gruppe`, `Beschreibg`, `Aktiv` ) VALUES( NULL, 6, '$desc', true )" ;
				}
...
// Formeln laden
	...
	$liste = array( ) ;
	$abfrage = "SELECT * FROM $db_ugsource WHERE Aktiv AND Gruppe = 6 ORDER BY Beschreibg" ;
	$ergebnis = mysql_query( $abfrage )  OR die( "Error: $abfrage <br>". mysql_error() ) ;
